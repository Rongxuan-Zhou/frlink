...
int main(int argc, char** argv) {
  ...
  if (rtcli.help) {
    std::cout << "Usage: " << argv[0]
              << " <robot-ip> [--port P] [--alpha A] [--K-t Kt] [--K-r Kr]\n"
                 "       [--load-mass M] [--load-com-x X] [--load-com-y Y] [--load-com-z Z]\n"
                 "       [--load-ixx I] [--load-iyy I] [--load-izz I] [--clip-trans M] [--clip-rot R]\n"
                 "       [--no-sing-scale] [--no-jlim] [--kz K] [--klock-rot K]\n"
                 "       [--null-kp KP] [--null-kd KD] [--null-mode init|pusht-side|elbow-up]\n"
              << rt::rt_flags_help();
    return 0;
  }
  ...
  // 载荷默认 = 常规 teleop 夹爪标定值 (m=0.25, COM_z=0.05)。
  // ⚠️ 不传 = 常规 teleop 行为字节不变（用户要求勿改常规 payload）。
  // PushT 无夹爪+pushtstick 配置经 calibrate_payload 实测最优 m≈0.0
  // (任何正质量都让 |F_ext| 单调变大 → 0.25 多补幽灵重力上托力矩
  //  → dead-man 慢漂飞天)，故 launch_pusht.sh 传 --load-mass 0.0。
  double load_mass = 0.25;
  double load_com_z = 0.05;
  // Phase 0 load-ID additions: COM x/y + inertia diagonal — full setLoad
  // parameter exposure. Defaults match prior hardcoded values for backward
  // compat (legacy scripts/behavior unchanged). For thin push-rod use:
  // measure mass + length L, set COM_z=L/2 + I_xx=I_yy=(1/3)mL^2.
  double load_com_x = 0.0;
  double load_com_y = 0.0;
  double load_ixx = 1e-4;
  double load_iyy = 1e-4;
  double load_izz = 1e-4;
  // SERL (rail-berkeley) 误差钳：把阻抗 error 向量分量钳死 → 力矩有界
  // → 设定点跑飞(伸展 model bias / dead-man)也不可能飞天，同时保留
  // torque 阻抗全部优点(平滑/无 CartesianPose 连续性 reflex/柔顺/高刚度)。
  // 默认 1e9 = 不钳 = 常规 teleop 行为字节不变；PushT 由 launch 传小值开启。
  double clip_trans = 1e9;   // m，平移 error 单分量上限
  double clip_rot = 1e9;     // rad，旋转 error 单分量上限
  // PushT 专用：关掉两个对约束平面任务有害的"智能"特性（默认 off=常规不变）
  //  --no-sing-scale: 不按 manipulability 塌 K（伸展位 K 塌→jlim 漏进 task 飞天）
  //  --no-jlim:       不加关节限位回避零空间项（DLS 漏 5-10% 进 task = 飞天源）
  bool no_sing_scale = false;
  bool no_jlim = false;
  // 各向异性刚度（PushT 严格锁 z + 法兰平行）。默认 <0 = off = 各向同性 = 常规不变。
  double K_z = -1.0;        // z 轴平移刚度（高值=严格锁高度）
  double K_lock_rot = -1.0; // 姿态刚度（高值=严格锁法兰平行桌面）
  // 零空间姿态维持（null-space posture control）
  // 目标 = servo 启动时的 init.q，整个任务过程中保持初始关节配置（侧推构型等）
  // 默认 0 = 关闭（行为与之前完全一致）；建议值 null_kp=20, null_kd=1
  double null_kp = 0.0;     // 零空间刚度 Nm/rad
  double null_kd = 1.0;     // 零空间阻尼 Nm/(rad/s)
  // null target 来源：init=启动时关节角(默认) | pusht-side=DAIRLab肘高构型
  //   pusht-side: 把 J2/J4/J6 零空间拉向肘高(避奇异/减jitter)；J1 由主任务带到EE朝向
  std::string null_mode = "init";
  for (int i = 2; i < argc; i += 2) {
    ...
    else if (flag == "--load-mass") load_mass = std::stod(argv[i + 1]);
    else if (flag == "--load-com-z") load_com_z = std::stod(argv[i + 1]);
    else if (flag == "--load-com-x") load_com_x = std::stod(argv[i + 1]);
    else if (flag == "--load-com-y") load_com_y = std::stod(argv[i + 1]);
    else if (flag == "--load-ixx") load_ixx = std::stod(argv[i + 1]);
    else if (flag == "--load-iyy") load_iyy = std::stod(argv[i + 1]);
    else if (flag == "--load-izz") load_izz = std::stod(argv[i + 1]);
    else if (flag == "--clip-trans") clip_trans = std::stod(argv[i + 1]);
    else if (flag == "--clip-rot") clip_rot = std::stod(argv[i + 1]);
    else if (flag == "--no-sing-scale") { no_sing_scale = true; i--; }
    else if (flag == "--no-jlim") { no_jlim = true; i--; }
    else if (flag == "--kz") K_z = std::stod(argv[i + 1]);
    else if (flag == "--klock-rot") K_lock_rot = std::stod(argv[i + 1]);
    else if (flag == "--null-kp") null_kp = std::stod(argv[i + 1]);
    else if (flag == "--null-kd") null_kd = std::stod(argv[i + 1]);
    else if (flag == "--null-mode") null_mode = argv[i + 1];
  }
  ...
  try {
    ...
    // 默认 0.25/0.05 = 常规 teleop 夹爪标定（不传 CLI 时行为不变）；
    // PushT 经 launch_pusht.sh 传 --load-mass 0.0 覆盖（无夹爪+stick 实测）。
    const double kMLoad = load_mass;
    std::array<double, 3> kCom{load_com_x, load_com_y, load_com_z};
    std::array<double, 9> kInertia{load_ixx, 0, 0,  0, load_iyy, 0,  0, 0, load_izz};
    robot.setLoad(kMLoad, kCom, kInertia);
    std::cerr << "  setLoad: m=" << kMLoad << " kg, COM=("
              << load_com_x << "," << load_com_y << "," << load_com_z << ") m, "
              << "I_diag=(" << load_ixx << "," << load_iyy << "," << load_izz << ") kg*m^2"
              << ((load_mass == 0.25 && load_com_x == 0.0 && load_com_y == 0.0
                   && load_com_z == 0.05 && load_ixx == 1e-4 && load_iyy == 1e-4
                   && load_izz == 1e-4) ? " (default/gripper)"
                                         : " (PushT override)") << std::endl;
    ...
    {   // q(7)+dq(7)+tau_J(7)+tau_ext(7) → franka_joint_state.txt  (供姿态诊断)
      Joint28 j0{};
      for (int i = 0; i < 7; ++i) {
        j0[i] = init.q[i];
        j0[7 + i] = init.dq[i];
        j0[14 + i] = init.tau_J[i];
        j0[21 + i] = init.tau_ext_hat_filtered[i];
      }
      sink.emit("franka_joint_state.txt", rt::format_doubles(j0.data(), 28));
    }

    // 零空间姿态目标；null_kp=0 时此项无效
    //   init       : 启动时的 init.q（维持当前配置）
    //   pusht-side : DAIRLab 肘高构型 [2.191,1.1,-1.33,-2.22,1.30,2.02,0.08]
    //                J1=125° 会被主任务(EE朝向)覆盖→实际J1跟EE；J2/J4/J6 被零空间拉向肘高
    Eigen::Matrix<double, 7, 1> q_null_target;
    if (null_mode == "pusht-side") {
      q_null_target << 2.191, 1.1, -1.33, -2.22, 1.30, 2.02, 0.08;
    } else if (null_mode == "elbow-up") {
      // J6-only：其余关节维持 init（EE/z 由主任务锁定，不撞桌），只把 J6 在零空间
      // 从靠极限的 153° 引向居中的 119°(2.074 rad)。纯零空间运动，绝不动 EE 高度。
      for (int i = 0; i < 7; ++i) q_null_target[i] = init.q[i];
      q_null_target[5] = 2.074;   // J6 → 119°（居中，远离 215°极限）
    } else {
      for (int i = 0; i < 7; ++i) q_null_target[i] = init.q[i];
    }
    if (null_kp > 0.0) {
      std::cout << "  null-space posture: kp=" << null_kp << " kd=" << null_kd
                << "  q_target=[";
      for (int i = 0; i < 7; ++i)
        std::cout << q_null_target[i] << (i < 6 ? "," : "]\n");
    }
    ...
    // 当前 EE / 关节 / 腕力共享：writer = control tick, reader = 各 writer 线程（lock-free）。
    // 每个 writer 线程：sleep → read → 共享 formatter → sink（文件 + FRST1 datagram）。
    rt::TripleBuffer<Pose16> current_ee(init_pose);
    rt::TripleBuffer<Joint28> current_joint;
    rt::TripleBuffer<Wrench6> current_wrench;
    std::thread ee_writer_thr([&]() {          // 20Hz
      rt::set_thread_name("ee_writer");
      rt::run_periodic_emit<16>(g_stop, 50, "franka_current_ee.txt", current_ee, sink);
    });
    std::thread joint_writer_thr([&]() {       // 10Hz
      rt::set_thread_name("joint_writer");
      rt::run_periodic_emit<28>(g_stop, 100, "franka_joint_state.txt", current_joint, sink);
    });
    std::thread wrench_writer_thr([&]() {      // 20Hz
      rt::set_thread_name("wrench_writer");
      rt::run_periodic_emit<6>(g_stop, 50, "franka_wrench.txt", current_wrench, sink);
    });
    ...
    // 奇异遥测节流（方案3 精确定 R）：r>0.45 中远端节流 100ms 打印一次
    int64_t last_telem_log_ms = 0;
    ...
    auto control_cb = [&](const franka::RobotState& state,
      ...
      // 把关节角暴露给诊断 writer（q+dq+tau_J+tau_ext）
      {
        std::array<double, 28> jbuf;
        for (int i = 0; i < 7; ++i) {
          jbuf[i] = state.q[i];
          jbuf[7 + i] = state.dq[i];
          jbuf[14 + i] = state.tau_J[i];
          jbuf[21 + i] = state.tau_ext_hat_filtered[i];
        }
      ...
      // ── SERL 误差钳（默认 1e9=off，PushT 由 --clip-trans/--clip-rot 开启）──
      // 钳死 error 各分量 → tau = Jᵀ(−K·error−...) 有界 → 设定点离机器人
      // 再远，力矩也封顶 → 物理上不可能飞天/力矩爆炸；torque 模式无
      // CartesianPose 连续性 reflex。这是 Franka 真机 RL/teleop 业界标准做法。
      for (int ci = 0; ci < 3; ++ci) {
        error(ci)     = std::clamp(error(ci),     -clip_trans, clip_trans);
        error(ci + 3) = std::clamp(error(ci + 3), -clip_rot,   clip_rot);
      }
      ...
      current_wrench.write(state.O_F_ext_hat_K);  // W1 exposer: persist only
      ...
      double k_sing_scale = no_sing_scale
          ? 1.0
          : std::clamp((w - w_min) / (w_full - w_min), 0.0, 1.0);
      ...
      // 各向异性刚度（PushT 选择性柔顺）：x,y=K_t_now 柔顺跟手；
      // z 用 K_z_now（默认=K_t，PushT 传高值严格锁高度）；
      // 姿态用 K_rlock_now（默认=K_r，PushT 传高值严格锁法兰平行）。
      // 默认 K_z<0 / K_lock_rot<0 → 退化为各向同性 = 常规 teleop 行为不变。
      double K_z_now = (K_z > 0.0)
          ? K_z * ((1.0 - contact_alpha) + contact_alpha * K_t_contact_ratio) * k_sing_scale
          : K_t_now;
      K_z_now = std::max(K_z_now, 10.0);
      double K_rlock_now = (K_lock_rot > 0.0)
          ? std::max(K_lock_rot * ((1.0 - contact_alpha) + contact_alpha * K_r_contact_ratio) * k_sing_scale, 1.0)
          : K_r_now;

      // ── 遥测（方案3 + z 漂升诊断）：r>0.45 节流 100ms 打印。含 K_z_now/
      //    K_rlock_now（z 锁 / 姿态锁实际刚度）→ 定位 z 漂升 vs K_z 关系。
      //    kIgnore 非硬 RT + 同款 emergency std::cerr 已在产 + 行缓冲 → 可接受。
      {
        double r_xy = std::sqrt(position.x() * position.x() +
                                position.y() * position.y());
        if (r_xy > 0.45 && now_ms - last_telem_log_ms > 100) {
          last_telem_log_ms = now_ms;
          std::cerr << "🌀 TELEM t=" << tick
                    << " EE=(" << position.x() << "," << position.y()
                    << "," << position.z() << ") r=" << r_xy
                    << " w=" << w << " k_sing=" << k_sing_scale
                    << " K_t=" << K_t_now << " K_z=" << K_z_now
                    << " K_rlock=" << K_rlock_now
                    << " |dq|=" << dq.norm()
                    << " |F|=" << F_force_norm << std::endl;
        }
      }

      Eigen::Vector3d kt_diag(K_t_now, K_t_now, K_z_now);
      stiff_now.topLeftCorner<3, 3>() = kt_diag.asDiagonal();
      stiff_now.bottomRightCorner<3, 3>() = K_rlock_now * Eigen::Matrix3d::Identity();
      // critical damping = 2*sqrt(K*M_eff)，逐轴匹配各自刚度避免欠阻尼抖振
      Eigen::Vector3d dt_diag(2.0 * std::sqrt(K_t_now * M_eff_t),
                              2.0 * std::sqrt(K_t_now * M_eff_t),
                              2.0 * std::sqrt(K_z_now * M_eff_t));
      damp_now.topLeftCorner<3, 3>() = dt_diag.asDiagonal();
      damp_now.bottomRightCorner<3, 3>() = 2.0 * std::sqrt(K_rlock_now * M_eff_r) * Eigen::Matrix3d::Identity();
      ...
      Eigen::Matrix<double, 7, 1> tau_jlim_ns;
      if (no_jlim) tau_jlim_ns.setZero();
      else tau_jlim_ns = N_proj * tau_jlim_raw;

      // 零空间姿态维持：把关节拉向 init 时记录的 q_null_target（侧推构型等）
      // τ_posture = N·(kp·(q_target−q) − kd·dq)，投影保证不干扰主任务
      Eigen::Matrix<double, 7, 1> tau_posture;
      if (null_kp > 0.0) {
        Eigen::Map<const Eigen::Matrix<double, 7, 1>> q(state.q.data());
        Eigen::Map<const Eigen::Matrix<double, 7, 1>> dq_vec(state.dq.data());
        tau_posture = N_proj * (null_kp * (q_null_target - q) - null_kd * dq_vec);
      } else {
        tau_posture.setZero();
      }

      Eigen::Matrix<double, 7, 1> tau_d = tau_task + tau_jlim_ns + tau_posture + coriolis;
    ...
    if (joint_writer_thr.joinable()) joint_writer_thr.join();
    if (wrench_writer_thr.joinable()) wrench_writer_thr.join();
