...
| `franky_tools/` | franky-based host-native scripts run through `bin/franka-py`: `goto_home.py` (what `franka-ctl goto home` runs: lifts clear of the table first and FK-checks the joint path via `franky_helpers.path_min_z`), payload calibration, joint and gripper wiggles. Direct-FCI tools: run only while no servo is active. |
...
5. `teleop/build.sh` — runs `docker/build-inside.sh` inside the image as the calling user:
   ...
   `gripper_cmd`, `fk_probe`), the `kIgnore` communication test,
...
| `goto home` | `franky_tools/goto_home.py` (via `bin/franka-py`) to the factory-ready pose at speed 0.10: lifts to the table-clearance height first when starting near the table and FK-checks the joint path before moving. Refuses if a servo is active. |
...
- The host repository also carries task-specific servo variants and their `franka-ctl` modes;
  they are not part of this export. Everything here is the regular 6-DOF pose servo with the
  Franka Hand, and the interface in `../docs/INTERFACE.md` is unchanged by that omission.
